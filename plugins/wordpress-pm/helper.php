...
function create_new_message_if_possible( $reciever_id , $text)
{
	...
	if( !empty($new_conversation) )
	{
		...
		$sql = $wpdb->query( $wpdb->prepare( "INSERT INTO ".$wpdb->prefix."pm_messages (`conv_id`, `sender_id`, `reciever_id`, `attachment_id`, `message`) VALUES (%d, %d, %d, %d, %s)", $new_conversation['id'], get_current_user_id(), $reciever_id, null, encrypt_decrypt($text, get_current_user_id())) );
