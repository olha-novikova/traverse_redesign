 ...
 */
class IfwPsn_Vendor_Twig_NodeVisitor_Optimizer implements IfwPsn_Vendor_Twig_NodeVisitorInterface
{
        ...
        if (!version_compare(phpversion(), '5.4.0RC1', '>=') && self::OPTIMIZE_VAR_ACCESS === (self::OPTIMIZE_VAR_ACCESS & $this->optimizers) && !$env->isStrictVariables() && !$env->hasExtension('sandbox')) {
            if ($this->inABody) {
                if (!$node instanceof IfwPsn_Vendor_Twig_Node_Expression) {
                    if (get_class($node) !== 'IfwPsn_Vendor_Twig_Node') {
    ...
    public function leaveNode(IfwPsn_Vendor_Twig_NodeInterface $node, IfwPsn_Vendor_Twig_Environment $env)
    {
        ...
        if (self::OPTIMIZE_VAR_ACCESS === (self::OPTIMIZE_VAR_ACCESS & $this->optimizers) && !$env->isStrictVariables() && !$env->hasExtension('sandbox')) {
            if ($node instanceof IfwPsn_Vendor_Twig_Node_Body) {
                ...
                if (!$expression && get_class($node) !== 'IfwPsn_Vendor_Twig_Node' && $prependedNodes = array_shift($this->prependedNodes)) {
                    $nodes = array();
                    foreach (array_unique($prependedNodes) as $name) {
                        $nodes[] = new IfwPsn_Vendor_Twig_Node_SetTemp($name, $node->getLine());
                    }
    ...
    protected function optimizeVariables(IfwPsn_Vendor_Twig_NodeInterface $node, IfwPsn_Vendor_Twig_Environment $env)
    {
        if ('IfwPsn_Vendor_Twig_Node_Expression_Name' === get_class($node) && $node->isSimple()) {
            ...
            return new IfwPsn_Vendor_Twig_Node_Expression_TempName($node->getAttribute('name'), $node->getLine());
        }
    ...
    protected function optimizePrintNode(IfwPsn_Vendor_Twig_NodeInterface $node, IfwPsn_Vendor_Twig_Environment $env)
    {
        ...
        if (
            $node->getNode('expr') instanceof IfwPsn_Vendor_Twig_Node_Expression_BlockReference ||
            $node->getNode('expr') instanceof IfwPsn_Vendor_Twig_Node_Expression_Parent
        ) {
