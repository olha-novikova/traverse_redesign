...
class IfwPsn_Vendor_Twig_ExpressionParser
{
    ...
    public function __construct(IfwPsn_Vendor_Twig_Parser $parser, array $unaryOperators, array $binaryOperators)
    {
        $this->parser = $parser;
        ...
        switch ($token->getType()) {
            ...
            default:
                if ($token->test(IfwPsn_Vendor_Twig_Token::PUNCTUATION_TYPE, '[')) {
                    ...
                    throw new IfwPsn_Vendor_Twig_Error_Syntax(sprintf('Unexpected token "%s" of value "%s"', IfwPsn_Vendor_Twig_Token::typeToEnglish($token->getType()), $token->getValue()), $token->getLine(), $this->parser->getFilename());
                }
        }
    ...
    public function parseStringExpression()
    {
        ...
        foreach ($nodes as $node) {
            $expr = new IfwPsn_Vendor_Twig_Node_Expression_Binary_Concat($expr, $node, $node->getLine());
        }
    ...
    public function parsePostfixExpression($node)
    {
        while (true) {
            $token = $this->parser->getCurrentToken();
            if ($token->getType() == IfwPsn_Vendor_Twig_Token::PUNCTUATION_TYPE) {
        ...
        switch ($name) {
            case 'parent':
                ...
                if (!count($this->parser->getBlockStack())) {
                    throw new IfwPsn_Vendor_Twig_Error_Syntax('Calling "parent" outside a block is forbidden', $line, $this->parser->getFilename());
                }

                if (!$this->parser->getParent() && !$this->parser->hasTraits()) {
                    throw new IfwPsn_Vendor_Twig_Error_Syntax('Calling "parent" on a template that does not extend nor "use" another template is forbidden', $line, $this->parser->getFilename());
                }

                return new IfwPsn_Vendor_Twig_Node_Expression_Parent($this->parser->peekBlockStack(), $line);
            case 'block':
                return new IfwPsn_Vendor_Twig_Node_Expression_BlockReference($this->parseArguments()->getNode(0), false, $line);
            case 'attribute':
                $args = $this->parseArguments();
                if (count($args) < 2) {
                    throw new IfwPsn_Vendor_Twig_Error_Syntax('The "attribute" function takes at least two arguments (the variable and the attributes)', $line, $this->parser->getFilename());
                }

                return new IfwPsn_Vendor_Twig_Node_Expression_GetAttr($args->getNode(0), $args->getNode(1), count($args) > 2 ? $args->getNode(2) : new IfwPsn_Vendor_Twig_Node_Expression_Array(array(), $line), IfwPsn_Vendor_Twig_Template::ANY_CALL, $line);
            default:
                if (null !== $alias = $this->parser->getImportedSymbol('function', $name)) {
                    $arguments = new IfwPsn_Vendor_Twig_Node_Expression_Array(array(), $line);
    ...
    public function parseSubscriptExpression($node)
    {
        ...
        $arguments = new IfwPsn_Vendor_Twig_Node_Expression_Array(array(), $lineno);
        $type = IfwPsn_Vendor_Twig_Template::ANY_CALL;
        if ($token->getValue() == '.') {
            $token = $stream->next();
            if (
                $token->getType() == IfwPsn_Vendor_Twig_Token::NAME_TYPE
                ||
                $token->getType() == IfwPsn_Vendor_Twig_Token::NUMBER_TYPE
                ||
                ($token->getType() == IfwPsn_Vendor_Twig_Token::OPERATOR_TYPE && preg_match(IfwPsn_Vendor_Twig_Lexer::REGEX_NAME, $token->getValue()))
            ) {
                $arg = new IfwPsn_Vendor_Twig_Node_Expression_Constant($token->getValue(), $lineno);

                if ($stream->test(IfwPsn_Vendor_Twig_Token::PUNCTUATION_TYPE, '(')) {
                    $type = IfwPsn_Vendor_Twig_TemplateInterface::METHOD_CALL;
                    foreach ($this->parseArguments() as $n) {
                        $arguments->addElement($n);
                    }
                }
            } else {
                throw new IfwPsn_Vendor_Twig_Error_Syntax('Expected name or number', $lineno, $this->parser->getFilename());
            }
            ...
            if ($node instanceof IfwPsn_Vendor_Twig_Node_Expression_Name && null !== $this->parser->getImportedSymbol('template', $node->getAttribute('name'))) {
                if (!$arg instanceof IfwPsn_Vendor_Twig_Node_Expression_Constant) {
                    throw new IfwPsn_Vendor_Twig_Error_Syntax(sprintf('Dynamic macro names are not supported (called on "%s")', $node->getAttribute('name')), $token->getLine(), $this->parser->getFilename());
                }

                $node = new IfwPsn_Vendor_Twig_Node_Expression_MethodCall($node, 'get'.$arg->getAttribute('value'), $arguments, $lineno);
                $node->setAttribute('safe', true);
            ...
            if ($slice) {
                ...
                $arguments = new IfwPsn_Vendor_Twig_Node(array($arg, $length));
        ...
        while (!$stream->test(IfwPsn_Vendor_Twig_Token::PUNCTUATION_TYPE, ')')) {
            ...
            if ($namedArguments && $token = $stream->nextIf(IfwPsn_Vendor_Twig_Token::OPERATOR_TYPE, '=')) {
                if (!$value instanceof IfwPsn_Vendor_Twig_Node_Expression_Name) {
                    throw new IfwPsn_Vendor_Twig_Error_Syntax(sprintf('A parameter name must be a string, "%s" given', get_class($value)), $token->getLine(), $this->parser->getFilename());
                }
                ...
                if ($definition) {
                    ...
                    if (!$this->checkConstantExpression($value)) {
                        throw new IfwPsn_Vendor_Twig_Error_Syntax(sprintf('A default value for an argument must be a constant (a boolean, a string, a number, or an array).'), $token->getLine(), $this->parser->getFilename());
                    }
                } else {
                    $value = $this->parseExpression();
                }
    ...
    public function parseAssignmentExpression()
    {
        $targets = array();
        while (true) {
            $token = $this->parser->getStream()->expect(IfwPsn_Vendor_Twig_Token::NAME_TYPE, null, 'Only variables can be assigned to');
            if (in_array($token->getValue(), array('true', 'false', 'none'))) {
                throw new IfwPsn_Vendor_Twig_Error_Syntax(sprintf('You cannot assign a value to "%s"', $token->getValue()), $token->getLine(), $this->parser->getFilename());
            }
            $targets[] = new IfwPsn_Vendor_Twig_Node_Expression_AssignName($token->getValue(), $token->getLine());
    ...
    protected function getFunctionNodeClass($name, $line)
    {
        ...
        if (false === $function = $env->getFunction($name)) {
            ...
            throw new IfwPsn_Vendor_Twig_Error_Syntax($message, $line, $this->parser->getFilename());
        }
