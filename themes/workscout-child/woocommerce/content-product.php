...
$classes = array();
	...
	if($product->product_type == "job_package" || $product->product_type == "resume_package") { ?>

	<li <?php post_class( $classes ); ?> package="<?php the_ID(); ?>">
		<?php
		...
		?>
		<?php if($product->product_type == "job_package" || $product->product_type == "resume_package") { ?>
...
<!--				<a href="--><?php //echo esc_url( $link ); ?><!--" class="button"><i class="fa fa-shopping-cart"></i> --><?php //echo $label; ?><!--</a>-->
		...
		<?php	
		if(is_archive()) {
			echo '<p class="estimate" style="display:none">';
