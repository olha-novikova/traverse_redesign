...
<div class="content">
    ...
    if(isset($_GET['success']) && $_GET['success'] == 1 && is_user_logged_in())  {
        ...
        <?php }
    }elseif(isset($_GET['saved']) && $_GET['saved'] == 1 && is_user_logged_in()){
        $canidate_dashboard_page = get_permalink( get_option( 'resume_manager_candidate_dashboard_page_id' ) );
        ?>
        ...
        jQuery(document).ready(function ($) {
            ...
            jQuery.magnificPopup.open({
                items: {
                    src: '<div id="singup-dialog" class="small-dialog zoom-anim-dialog apply-popup">'+
                        '<div class="small-dialog-headline"><h2><?php esc_html_e("Success!","workscout"); ?></h2></div>'+
                        '<div class="small-dialog-content"><p class="margin-reset"><?php esc_html_e("Account details changed successfully. Thank you!","woocommerce"); ?></p><p style="text-align: right;"><a class="button" href="<?php echo $canidate_dashboard_page?>">OK</a></p></div>'+
                        '</div>',
                    type: 'inline'
                }
            });
...
<!--            window.location.href = "--><?php //echo $canidate_dashboard_page;?><!--";-->
<!--        }, 2000);-->
    </script>
