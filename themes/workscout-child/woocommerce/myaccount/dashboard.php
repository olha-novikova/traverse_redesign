...
<p class="woocommerce-dashboard-welcome">
	<?php
		echo sprintf( esc_attr__( 'From your account dashboard you can view your %1$srecent pitches%2$s, manage your %3$sProfile and portfolio%2$s and %4$sedit your password and account details%2$s.', 'workscout' ), 
...
<p>
	<?php
		...
		if(class_exists( 'WP_Job_Manager_Applications' )) {
			...
			echo sprintf( 
				esc_attr__( 'To check your Job Listings and Applications visit %1$sMy Listings%2$s.', 'workscout' ),
			...
			echo sprintf( 
				esc_attr__( 'To check your Resumes and Past Applications visit %1$sMy Portfolios%2$s.', 'workscout' ),
			...
			if ( in_array( 'employer', (array) $user->roles ) || in_array( 'administrator', (array) $user->roles ) ) : 
			echo sprintf( 
				esc_attr__( 'To check your Job Listings visit %1$sMy Listings%2$s.', 'workscout' ),
				'<a href="' . esc_url( get_permalink($employer_dashboard_page_id)) . '">', 
				'</a>'
				);
			endif;
			if ( in_array( 'candidate', (array) $user->roles ) || in_array( 'administrator', (array) $user->roles ) ) :
			echo sprintf( 
				esc_attr__( 'To check your Resumes visit %1$sMy Portfolios%2$s.', 'workscout' ),
				'<a href="' . esc_url( get_permalink($candidate_dashboard_page_id)) . '">', 
				'</a>'
				);
			endif;
