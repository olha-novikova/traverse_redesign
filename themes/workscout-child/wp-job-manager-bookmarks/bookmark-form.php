<?php global $wp; ?>
	...
	<?php if ( $is_bookmarked ) : ?>
		...
		<?php $bookmarks_page = ot_get_option('pp_bookmarks_page'); 
	...
	<!--<a class="bookmark-notice small-dialog popup-with-zoom-anim button dark" href="#bookmark-dialog"><i class="fa fa-star"></i>  <?php //printf( esc_html__( 'Bookmark This %s', 'workscout' ), ucwords( $post_type->labels->singular_name ) ); ?></a>-->
		<a class="bookmark-notice small-dialog popup-with-zoom-anim button dark" href="#bookmark-dialog"><i class="fa fa-star"></i>  <?php printf( esc_html__( 'Save To Favorites', 'workscout' ), ucwords( $post_type->labels->singular_name ) ); ?></a>
	<?php endif; ?>
	...
	<form method="post" action="<?php echo defined( 'DOING_AJAX' ) ? '' : esc_url( remove_query_arg( array( 'page', 'paged' ), add_query_arg( $wp->query_string, '', home_url( $wp->request ) ) ) ); ?>" class="job-manager-form wp-job-manager-bookmarks-form">
		...
		<div class="small-dialog-content">
			<p><label for="bookmark_notes"><?php esc_html_e( 'Notes:', 'workscout' ); ?></label><textarea name="bookmark_notes" id="bookmark_notes" cols="25" rows="3"><?php echo esc_textarea( $note ); ?></textarea></p>
			<p>
